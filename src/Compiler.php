...
final class Compiler
{
    ...
    private function compileWith(?array $withCondition): array
    {
        ...
        if ($withCondition) {
            ...
            if ($this->mapHasKey($withCondition, self::$with)) {
                $without = []; // cancel the default
                $list = $this->coerceList($this->libMapGet([$withCondition, self::$with]));
            ...
            if ($this->mapHasKey($withCondition, self::$without)) {
                $without = []; // cancel the default
                $list = $this->coerceList($this->libMapGet([$withCondition, self::$without]));
...
EOL;
     ...
     * @return array|Number
     */
    private function coerceMap($item)
    {
    ...
    */
    ...
    private static $libInvert = ['color', 'weight:1'];
...
will be an error in future versions of Sass.\n         on line $line of $fname";
    ...
    private static $libMapGet = ['map', 'key'];
    ...
    private static $libMapHasKey = ['map', 'key'];
