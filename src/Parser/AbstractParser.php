...
abstract class AbstractParser
{
    ...
    protected function tryUrl(): ?string
    {
        ...
        while (true) {
            ...
            if ($next === '%' || $next === '&' || $next === '#' || ($nextCharCode >= \ord('*') && $nextCharCode <= \ord('~')) || $nextCharCode >= 0x80) {
                $buffer .= $this->scanner->readUtf8Char();
            } elseif ($next === '\\') {
                $buffer .= $this->escape();
    ...
    protected function escapeCharacter(): string
    {
        ...
        return $this->scanner->readUtf8Char();
    }

    /**
     * @phpstan-param callable(string): bool $condition
     */
    ...
    {
        $matches = function (string $actual) use ($character, $caseSensitive) {
            if ($caseSensitive) {
     ...
     *
     * @phpstan-param callable(): void $consumer
     */
