 ...
 * @internal
 */
abstract class StylesheetParser extends AbstractParser
{
    /**
     ...
     *
     * This is `null` unless {@see $inMixin} is `true`.
     *
     * @var bool|null
     */
    private $mixinHasContent;
    ...
    public function __construct(string $contents, ?LoggerInterface $logger = null, ?string $sourceUrl = null)
    {
        ...
        $this->numberCallable = \Closure::fromCallable([$this, 'number']);
    }
     ...
     * root of the stylesheet.
     *
     * @phpstan-param callable(): Statement $child
     */
     ...
     *
     * @phpstan-return array{SupportsCondition|null, Interpolation|null}
     */
    ...
    private function mixinRule(int $start): MixinRule
    {
        ...
        return $this->withChildren($this->statementCallable, $start, function (array $children, FileSpan $span) use ($name, $arguments, $precedingComment) {
            assert($this->mixinHasContent !== null);
            $hadContent = $this->mixinHasContent;
            $this->inMixin = false;
            $this->mixinHasContent = null;

            return new MixinRule($name, $arguments, $span, $children, $hadContent, $precedingComment);
        });
    }
     ...
     *
     * @phpstan-param (callable(): bool)|null $until
     */
    protected function expression(?callable $until = null, bool $singleEquals = false, bool $bracketList = false): Expression
    {
         ...
         * of `$operators[n]`.
         *
         * @phpstan-var list<Expression>|null $operands
         */
        $operands = null;
         ...
         */
        $allowSlash = $this->lookingAtNumber();
         ...
         */
        $resetState = function () use (&$commaExpressions, &$spaceExpressions, &$operators, &$operands, &$allowSlash, &$singleExpression, $start) {
            $commaExpressions = null;
            $spaceExpressions = null;
            $operators = null;
            $operands = null;
        ...
        $resolveOneOperation = function () use (&$operands, &$operators, &$singleExpression, &$allowSlash) {
            ...
            if ($operator !== BinaryOperator::DIVIDED_BY) {
                $allowSlash = false;
            }

            $operand = array_pop($operands);
            assert($operand !== null, 'The list of operands must not be empty');

            if ($allowSlash && !$this->inParentheses) {
                $singleExpression = BinaryOperationExpression::slash($operand, $singleExpression);
            } else {
                $singleExpression = new BinaryOperationExpression($operator, $operand, $singleExpression);
            }
        };

        $resolveOperations = function () use (&$operators, $resolveOneOperation) {
            if ($operators === null) {
                return;
            }
        ...
        $addSingleExpression = function (Expression $expression, bool $number = false) use (&$singleExpression, &$allowSlash, &$spaceExpressions, $resetState, $resolveOperations) {
            if ($singleExpression !== null) {
        ...
        $addOperator =
            /**
             * @phpstan-param BinaryOperator::* $operator
             */
            function (string $operator) use (&$allowSlash, &$operators, &$operands, &$singleExpression, $resolveOneOperation) {
                ...
                $this->whitespace();
                $allowSlash = $allowSlash && $this->lookingAtNumber();
                $singleExpression = $this->singleExpression();
                $allowSlash = $allowSlash && $singleExpression instanceof NumberExpression;
            };

        $resolveSpaceExpressions = function () use (&$spaceExpressions, &$singleExpression, $resolveOperations) {
            $resolveOperations();
            ...
            switch ($first) {
                ...
                case '-':
                    ...
                    if ((Character::isDigit($next) || $next === '.') && ($singleExpression === null || Character::isWhitespace($this->scanner->peekChar(-1)))) {
                        $addSingleExpression($this->number(), true);
     ...
     */
    private function hexColorContents(): array // TODO change that type to SassColor
    {
        ...
        $color = [Type::T_COLOR, $red, $green, $blue];
    ...
    private function unicodeRange(): StringExpression
    {
        ...
        $firstRangeLength = 0;
        while ($this->scanCharIf(static function ($char) {
            return $char !== null && Character::isHex($char);
        })) {
            $firstRangeLength++;
        }
    ...
    protected function identifierLike(): Expression
    {
        ...
        if ($plain !== null) {
            ...
            if ($this->scanner->peekChar() !== '(') {
                ...
                $color = Colors::colorNameToRGBa($lower);
        ...
        switch ($this->scanner->peekChar()) {
            case '.':
                ...
                $name = new Interpolation([$this->publicIdentifier()], $this->scanner->spanFrom($beforeName));

                // TODO remove this when implementing modules
                $this->error('Sass modules are not implemented yet.', $this->scanner->spanFrom($start));
                // return new FunctionExpression($name, $this->argumentInvocation(), $this->scanner->spanFrom($start), $plain);

            case '(':
                return new FunctionExpression($identifier, $this->argumentInvocation(), $this->scanner->spanFrom($start));
    ...
    protected function trySpecialFunction(string $name, int $start): ?Expression
    {
        ...
        switch ($normalized) {
            ...
            case 'max':
                ...
                if (!$this->tryMinMaxContents($buffer)) {
     ...
     */
    private function tryMinMaxContents(InterpolationBuffer $buffer, bool $allowComma = true): bool
    {
        while (true) {
            $next = $this->scanner->peekChar();
            ...
            switch ($next) {
                ...
                case '9':
                    try {
                        $buffer->write($this->rawText($this->numberCallable));
                    } catch (FormatException $e) {
                        return false;
                    }
                    break;
                ...
                case 'C':
                    switch ($this->scanner->peekChar(1)) {
                        case 'a':
                        case 'A':
                            if (!$this->tryMinMaxFunction($buffer, 'calc')) {
                                return false;
                            }
                            break;
                        ...
                        case 'l':
                        case 'L':
                            if (!$this->tryMinMaxFunction($buffer, 'clamp')) {
                                return false;
                            }
                            break;
                    }
                ...
                case 'e':
                case 'E':
                    if (!$this->tryMinMaxFunction($buffer, 'env')) {
                        return false;
                    }
                    break;
                ...
                case 'v':
                case 'V':
                    if (!$this->tryMinMaxFunction($buffer, 'var')) {
                        return false;
                    }
                    break;
                ...
                case '(':
                    $buffer->write($this->scanner->readChar());
                    if (!$this->tryMinMaxContents($buffer, false)) {
                        return false;
                    }
                    break;
                ...
                case 'M':
                    ...
                    if ($this->scanIdentChar('i')) {
                        if (!$this->scanIdentChar('n')) {
                            return false;
                        }
                        $buffer->write('min(');
                    } elseif ($this->scanIdentChar('a')) {
                        if (!$this->scanIdentChar('x')) {
                            return false;
                        }
                        $buffer->write('max(');
                    } else {
     ...
     */
    private function tryMinMaxFunction(InterpolationBuffer $buffer, string $name): bool
    {
        if (!$this->scanIdentifier($name)) {
            return false;
        }
    ...
    private function tryUrlContents(int $start, ?string $name = null): ?Interpolation
    {
        ...
        while (true) {
            ...
            if ($next === '!' || $next === '%' || $next === '&' || (\ord($next) >= \ord('*') && \ord($next) <= \ord('~')) || \ord($next) >= 0x80) {
                $buffer->write($this->scanner->readUtf8Char());
            } elseif ($next === '\\') {
    ...
    protected function dynamicUrl(): Expression
    {
        ...
        return new FunctionExpression(new Interpolation(['url'], $this->scanner->spanFrom($start)), $this->argumentInvocation(), $this->scanner->spanFrom($start));
    }
     ...
     * @phpstan-template T
     * @phpstan-param callable(): Statement $child
     * @phpstan-param callable(Statement[], FileSpan): T $create
     * @phpstan-return T
     */
     ...
     *
     * @phpstan-param callable(): FileSpan $span
     */
     ...
     * @return Statement[]
     *
     * @phpstan-param callable(): ?Statement $statement
     */
